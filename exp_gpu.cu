...
extern float allocGPU(gputype **results, int n, int m){
...
extern float copyfromGPU(gputype **CPUresults, gputype **resultsGPU, int n, int m){
	cudaEvent_t start, finish;
	cudaEventCreate(&start);
	cudaEventCreate(&finish);
...
extern void freeGPU(gputype **results){
	cudaFree(*results);
}
        ...
        if (n==0) {
                ...
                if (mem[tid][9]>1.0) {
                        ...
                        for (i=1;i<=maxIterations;i++) {
                                ...
                                mem[tid][6]*=mem[tid][4];
                                if (fabsf(mem[tid][4]-1.0)<=constants[1]) {
                        ...
                        for (i=1;i<=maxIterations;i++) {
                                ...
                                mem[tid][8]+=mem[tid][4];
                                if (fabsf(mem[tid][4])<fabsf(mem[tid][8])*constants[1]) return mem[tid][8];
                        }
                        return mem[tid][8];
                }
        }
...
extern float gpu_standard(gputype *results, int n, int m, gputype a, gputype b, int maxIterations, bool simple){	
	dim3 dimBlock(N);	
...
extern float gpu_streams(gputype **CPUresults, int n, int m, gputype a, gputype b, int maxIterations, bool simple, int nostreams){	
	dim3 dimBlock(N);	
...
extern float gpu_mpi(gputype **CPUresults, int n, int m, gputype a, gputype b, int maxIterations, int *CPUrank, int jump, bool simple){	
	int i;
	...
	*CPUrank=rank;
	...
	if(rank==0){
		...
		while(nval+jump-1<=n){
			...
			MPI_Recv(&((*CPUresults)[(index-1)*m]), m*jump, MPI_FLOAT, stat.MPI_SOURCE, 0, MPI_COMM_WORLD, &stat);
		...
		for(i=1;i<=no_cards;i++){
			...
			MPI_Recv(&((*CPUresults)[(index-1)*m]), m*jump, MPI_FLOAT, i, 0, MPI_COMM_WORLD, &stat);
	...
	// Slaves
		...
		if(simple){
			...
			while(true){
				...
				MPI_Send(*CPUresults, m*jump, MPI_FLOAT, 0, 0, MPI_COMM_WORLD);
			}
		}
	...
	//MPI_Finalize();		// commented out as can't finalise before runnning the doubles version
...
extern float gpu_dynamic(gputype *results, int n, int m, gputype a, gputype b, int maxIterations, bool simple){	
	dim3 dimBlock(N);	
...
// *************************************
extern float gpu_streams_dynamic(gputype **CPUresults, int n, int m, gputype a, gputype b, int maxIterations, bool simple, int nostreams){	
	dim3 dimBlock(N);	
...
// **** DYNAMIC & MPI VERSION ******
// *************************************
extern float gpu_dynamic_mpi(gputype **CPUresults, int n, int m, gputype a, gputype b, int maxIterations, int *CPUrank, int jump, bool simple){	
	int i;
...
// **** STREAMS & MPI VERSION ******
// *************************************
extern float gpu_streams_mpi(gputype **CPUresults, int n, int m, gputype a, gputype b, int maxIterations, int *CPUrank, int jump, bool simple, int nostreams){	
	int i;
...
// *************************************
extern float gpu_dynamic_streams_mpi(gputype **CPUresults, int n, int m, gputype a, gputype b, int maxIterations, int *CPUrank, int jump, bool simple, int nostreams){	
	int i;
